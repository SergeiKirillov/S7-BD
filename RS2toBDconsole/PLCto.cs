...
class PLCto
{
    ...
    #region Свойства определяющие настройки связи с контроллером
        private string name; //ip адресс контроллера
        ...
        try
        {
            ...
            else
            {
                ...
                //Console.WriteLine(D_tek_mot+" - "+h5w+" - "+speed4kl+" - "+Bw);
                    ...
                    if ((Time_Start != new DateTime()) && (H_work == 0) && (D_tek_mot>0.7)&&(speed4kl>2))
                    {
                        H_work = hw;
                        B_Work = Bw;
                    }
                    ...
                    if ((Time_Start != new DateTime()) && (H_work != 0) && (D_tek_mot < 0.610) && (D_tek_mot < D_pred_mot))
                    {
